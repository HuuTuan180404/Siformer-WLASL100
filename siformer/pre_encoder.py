...
class EncoderLayer(nn.Module):
    ...
    def forward(self, l_hand_x, r_hand_x, body_x):
        ...
        lh_self, _ = self.attention_lh(lh_norm, lh_norm, lh_norm)
        l_hand_x = l_hand_x + self.dropout_attn(lh_self)

        rh_norm = self.norm1_rh(r_hand_x)
        rh_self, _ = self.attention_rh(rh_norm, rh_norm, rh_norm)
        r_hand_x = r_hand_x + self.dropout_attn(rh_self)

        body_norm = self.norm1_body(body_x)
        body_self, _ = self.attention_body(body_norm, body_norm, body_norm)
        body_x = body_x + self.dropout_attn(body_self)

        # --- Pre-Norm: Communication ---
        lh_norm2 = self.norm2_lh(l_hand_x)
        rh_norm2 = self.norm2_rh(r_hand_x)

        lh_from_rh, _ = self.lh_from_rh_attn(lh_norm2, rh_norm2, rh_norm2)
        lh_fused = self.lh_fusion_layer(lh_from_rh)
        l_hand_x = l_hand_x + self.dropout_fusion(lh_fused)

        rh_from_lh, _ = self.rh_from_lh_attn(rh_norm2, lh_norm2, lh_norm2)
        rh_fused = self.rh_fusion_layer(rh_from_lh)
        r_hand_x = r_hand_x + self.dropout_fusion(rh_fused)

        # --- Pre-Norm: FFN ---
        lh_norm3 = self.norm3_lh(l_hand_x)
        rh_norm3 = self.norm3_rh(r_hand_x)
        body_norm3 = self.norm3_body(body_x)

        l_hand_x = l_hand_x + self.dropout_ffn(self.ffn_lh(lh_norm3))
        r_hand_x = r_hand_x + self.dropout_ffn(self.ffn_rh(rh_norm3))
        body_x = body_x + self.dropout_ffn(self.ffn_body(body_norm3))
...
class PBEEncoder(nn.TransformerEncoder):
    ...
    def forward(self, lh, rh, body):
        ...
        for i, mod in enumerate(self.layers):
            lh, rh, body = mod(lh, rh, body) # lh, rh, body: (L, B, D)
